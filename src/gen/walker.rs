...
impl Walker {
    pub fn new(start: IVec2) -> Self {
        Walker { current_location: start, steps_to_live: thread_rng().gen_range(3..=70) }
    }
    ...
    pub fn walk(&mut self, location: &mut ResMut<WorldCatacomb>) {
        ...
        let offset = IVec2 { 
            x: if is_x { rand::thread_rng().gen_range(-1..=1) } else { 0 }, 
            y: if !is_x { rand::thread_rng().gen_range(-1..=1) } else { 0 }
        };
...
pub fn walk_walker_generators(mut q_walkers: Query<&mut Walker>, mut location: ResMut<WorldCatacomb>) {
